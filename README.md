...
### 08-03-26
1. Airport diagrams are now fetched directly from the FAA using the current chart cycle, replacing the previously outdated 2024 FlightAware charts.

2. Added support for Instrument Approach Procedures (IAPs). IAP charts are now fully georeferenced.

3. Added flight path tracking, with rendered path lines for visualizing aircraft trajectories.

4. Added a 3D view for visualizing flight paths using CesiumJS.

### 07-01-26
1. Added a glyph-to-text fallback to the georeferencing pipeline for newer diagrams that do not contain embedded latitude/longitude text. These diagrams instead encode coordinates as vector glyphs. `georeferencing_with_glyph_fallback.py`
